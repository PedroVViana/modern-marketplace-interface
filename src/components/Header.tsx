...
export function Header() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    // Em um app real, você implementaria a funcionalidade de busca aqui
  };
    ...
    <header className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container flex h-16 items-center">
        <div className="mr-4 md:hidden">
          <Button variant="ghost" size="icon" onClick={toggleMenu} aria-label="Alternar Menu">
        ...
        <div className="flex items-center">
          <Link to="/" className="flex items-center space-x-2">
            <span className="font-bold text-2xl bg-primary text-primary-foreground px-2 py-1 rounded">TrocaFácil</span>
          </Link>
        </div>
            ...
            <nav className="flex flex-col space-y-6">
              ...
              <div className="border-t pt-6">
                ...
                <div className="space-y-3">
                  {['Eletrônicos', 'Veículos', 'Imóveis', 'Empregos', 'Móveis', 'Moda'].map((category) => (
                    <Link
                      key={category}
